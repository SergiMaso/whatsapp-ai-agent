...
class WeeklyDefaultsManager:
    ...
    def ensure_table_exists(self):
        """Crea la taula weekly_defaults si no existeix i inicialitza valors per defecte"""
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    # Inicialitzar valors per defecte si està buida
                    ...
                    if cursor.fetchone()[0] == 0:
                        ...
                        for day_num, day_name in enumerate(day_names):
                            cursor.execute("""
                                ...
                                VALUES (%s, %s, 'full_day', TIME '12:00:00', TIME '15:00:00', TIME '19:00:00', TIME '22:30:00')
                            """, (day_num, day_name))
                        
                        print("✅ Configuració setmanal per defecte inicialitzada correctament.")
                    else:
    ...
    def get_all_defaults(self):
        """Obté la configuració per defecte de tots els dies"""
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("""
    ...
    def get_default_for_day(self, day_of_week):
        """Obté la configuració per defecte d’un dia específic"""
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("""
    ...
    def update_default(self, day_of_week, status, lunch_start=None, lunch_end=None, 
        ...
        """
        Actualitza la configuració per defecte d’un dia de la setmana
        i aplica els canvis als dies futurs d’opening_hours sense configuració personalitzada.
        """
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    # Actualitzar la taula de configuració setmanal
                    cursor.execute("""
                    ...
                    # Aplicar canvis a opening_hours (dies futurs sense notes)
                    today = datetime.now().date()
                    postgres_dow = day_of_week + 1  # PostgreSQL usa 1=dilluns
                    ...
                    print(f"✅ Configuració del dia {day_of_week} actualitzada correctament.")
                    ...
                    return {
                        ...
                        'message': f'S’han actualitzat {days_updated} dies futurs.'
                    }
        except Exception as e:
